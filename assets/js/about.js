(function () {
      ...
      const PAGE_TITLES = {
        it: 'About Me | Loris Battaglini',
        en: 'About Me | Loris Battaglini'
      };
      ...
      const ENGLISH_TEXT = {
        ...
        aboutNavVision: 'Vision',
        aboutNavMindset: 'Mindset',
        aboutNavContact: 'Contact',
        aboutContactLink: 'Contact me',
        aboutHeroTitle: 'I build long-term results, not empty pages.',
        aboutHeroLead: 'My name is Loris Battaglini, and I am training as a full-stack web developer. Every day I study to help professionals and small projects turn an idea into a clear, credible, and concrete online presence.',
        aboutManifestoText: 'I believe in building before talking.',
        aboutVisionKicker: 'Where I am heading',
        aboutVisionTitle: 'From operational freelance work to building digital products.',
        aboutVisionParagraph1: 'In the short term I want to collaborate with professionals, creators, and small businesses that need a clear and credible online presence. At the same time, I am also open to collaborations with companies and development teams to grow in teamwork on more structured projects and keep raising my level.',
        aboutVisionParagraph2: 'In the medium to long term I want to expand the scope: not only services, but also MVPs and proprietary digital products. The goal is to build an autonomous, solid, and scalable career.',
        aboutPrinciplesKicker: 'How I think while building',
        aboutPrinciplesTitle: 'Three principles that guide every project.',
        aboutPrinciple1: 'Clarity before code',
        aboutPrinciple2: 'Structure that guides users',
        aboutPrinciple3: 'Clean code, ready to evolve',
        aboutBeforeLabel: 'Before',
        aboutAfterLabel: 'After',
        aboutCompareCaption: 'Move the slider left and right to compare before and after.',
        aboutMindsetKicker: 'Mindset',
        aboutMindsetTitle: 'Discipline and consistency: my work standard.',
        aboutMindsetParagraph1: 'I used to weigh around 130 kg. With discipline, training, and consistency, I brought my body down to 80 kg. I do not use it as a motivational story. I consider it operational proof: if I define a goal, I set a method and follow it.',
        aboutMindsetParagraph2: 'In development I apply the same logic: clear process, consistent execution, and progressive improvement.',
        aboutMindsetQuote: 'Results come when method is stronger than improvisation.',
        aboutTrustKicker: 'Why you can trust me',
        aboutTrustTitle: 'Because I work with method, transparency, and consistency.',
        aboutTrustParagraph: 'If you have an idea and want to turn it into an organized and credible web project, let\'s talk. I reply with initial feedback within 24 hours.',
        aboutTrustButton: 'Go to contact form'
      };
      const ENGLISH_ARIA_LABELS = {
        ...
        aboutManifestoAriaLabel: 'Personal manifesto',
        aboutPrinciplesAriaLabel: 'How I think when building',
        aboutCompareAriaLabel: 'Compare before and after photos'
      };
      ...
      const manifestoText = document.querySelector('.about-manifesto p');
      ...
      function isPhoneViewportNow() {
        return phoneViewportQuery ? phoneViewportQuery.matches : window.innerWidth <= 767;
      }

      function getManifestoForcedBreakIndex(text) {
        if (!text || !isPhoneViewportNow()) return -1;
        const targetWord = currentLanguage === 'en' ? 'before' : 'prima';
        const wordIndex = text.toLowerCase().indexOf(targetWord);
        if (wordIndex === -1) return -1;
        return wordIndex + targetWord.length;
      }

      function applyManifestoMobileBreak() {
        if (!manifestoText) return;
        const fullText = manifestoText.textContent.trim();
        if (!fullText) return;
        const breakIndex = getManifestoForcedBreakIndex(fullText);
        if (breakIndex <= 0 || breakIndex >= fullText.length) return;
        const firstPart = fullText.slice(0, breakIndex).trimEnd();
        const secondPart = fullText.slice(breakIndex).trimStart();
        manifestoText.textContent = '';
        manifestoText.appendChild(document.createTextNode(firstPart));
        manifestoText.appendChild(document.createElement('br'));
        manifestoText.appendChild(document.createTextNode(secondPart));
      }
      ...
      function applyTranslations() {
        ...
        document.querySelectorAll('[data-i18n]').forEach(function (node) {
          ...
          if (currentLanguage === 'en' && Object.prototype.hasOwnProperty.call(ENGLISH_TEXT, key)) {
            node.innerHTML = ENGLISH_TEXT[key];
            return;
          }
          node.innerHTML = italianSnapshot.text[key] || '';
        });
        ...
        document.querySelectorAll('[data-i18n-aria-label]').forEach(function (node) {
          ...
          if (currentLanguage === 'en' && Object.prototype.hasOwnProperty.call(ENGLISH_ARIA_LABELS, key)) {
            node.setAttribute('aria-label', ENGLISH_ARIA_LABELS[key]);
            return;
          }
          node.setAttribute('aria-label', italianSnapshot.aria[key] || '');
        });
      ...
      const compareBlocks = document.querySelectorAll('[data-before-after]');
      ...
      function navigateToTarget(targetId) {
        ...
        if (targetId.charAt(0) !== '#') return false;
      ...
      function focusSectionFromHash() {
        ...
        if (clearHashTimeoutId) {
          window.clearTimeout(clearHashTimeoutId);
        }
        clearHashTimeoutId = window.setTimeout(function () {
          clearHashTimeoutId = 0;
          clearHashFromUrl();
        }, 740);
      }
      ...
      function playManifestoTyping() {
        if (!manifestoText || manifestoText.dataset.typed === 'true') return;
        if (window.matchMedia('(prefers-reduced-motion: reduce)').matches) {
          applyManifestoMobileBreak();
          manifestoText.dataset.typed = 'true';
          return;
        }

        const fullText = manifestoText.textContent.replace(/\s+/g, ' ').trim();
        if (!fullText) return;
        const forcedBreakIndex = getManifestoForcedBreakIndex(fullText);

        const startDelay = 260;

        manifestoText.classList.add('is-typing');
        manifestoText.classList.add('manifesto-text-reveal');
        manifestoText.textContent = '';

        const charNodes = [];
        fullText.split('').forEach(function (char, index) {
          if (char === ' ') {
            ...
            manifestoText.appendChild(charSpan);
            charNodes.push(charSpan);
          }

          if (forcedBreakIndex !== -1 && index + 1 === forcedBreakIndex) {
            manifestoText.appendChild(document.createElement('br'));
          }
        });

        let totalDelay = startDelay;
        charNodes.forEach(function (charNode) {
          const charValue = charNode.textContent;
          window.setTimeout(function () {
            charNode.classList.add('is-visible');
          }, totalDelay);
          totalDelay += /[.,;:!?]/.test(charValue) ? 145 : 68;
        });

        const endDelay = totalDelay + 680;
        window.setTimeout(function () {
          manifestoText.classList.remove('is-typing');
          manifestoText.dataset.typed = 'true';
        }, endDelay);
      }

      function setupRevealOnScroll() {
        if (!aboutShell || revealTargets.length === 0) return;

        aboutShell.classList.add('reveal-ready');
        ...
        const observer = new IntersectionObserver(function (entries) {
          entries.forEach(function (entry) {
            ...
            if (entry.target.classList.contains('about-manifesto')) {
              playManifestoTyping();
            }
